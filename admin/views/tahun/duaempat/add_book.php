...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 if (isset($_FILES['cover']) && $_FILES['cover']['error'] === UPLOAD_ERR_OK) {
  ...
  try {
   ...
   echo "<script>console.log('Debug Objects: " . $coverPath . "' );</script>";
  } catch (Exception) {
   echo "<script>console.log('Debug Objects: " . $coverPath . "' );</script>";
  }
 }
 ...
 if (isset($_FILES['content']) && $_FILES['content']['error'] === UPLOAD_ERR_OK) {
  $fileType = mime_content_type($_FILES['content']['tmp_name']);
  ...
  $contentExt = pathinfo($_FILES['content']['name'], PATHINFO_EXTENSION);
  $contentFilename = uniqid('content_') . '.' . $contentExt;
  $contentPath = $contentFilename;
  move_uploaded_file($_FILES['content']['tmp_name'], $uploadDir . $contentFilename);
 }

 // Simpan ke database
 if ($bookModel->create($judul, $penerbit, $category_id, $coverPath, $contentPath)) {
  ...
  <div class="form-container">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
    ...
    <div class="content-uploads">
     ...
     <div class="file-upload-wrapper">
      <input type="file" id="content-file" name="content" class="file-upload-input" accept="application/pdf">
